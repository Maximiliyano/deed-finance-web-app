...
public sealed class CreateGoalCommandValidatorTests
{
    ...
    private static CreateGoalCommand ValidCommand => new(
        "Save for car", 10000m, CurrencyType.UAH, 0m, null, null);
    ...
    [Fact]
    public async Task Validate_ZeroCurrentAmount_PassesValidation()
    {
        var command = ValidCommand with { CurrentAmount = 0m };
        var result = await _validator.TestValidateAsync(command);
        result.ShouldNotHaveValidationErrorFor(c => c.CurrentAmount);
    }
    ...
    [Fact]
    public async Task Validate_NoteAtMaxLength_PassesValidation()
    {
